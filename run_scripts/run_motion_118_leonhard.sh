...
do
    ...
    mkdir -p $WORKING_DIR
    bsub -J "motion_118" -n $NUM_CPU_CORES -W $RUN_TIME -R "rusage[mem=$MEM_PER_CPU_CORE,ngpus_excl_p=$NUM_GPU_CORES]" -R "select[gpu_mtotal0>=10240]" <<ENDBSUB
    ...
    python -u dnri/experiments/motion_experiment.py --gpu --mode eval_fixedwindow --load_best_model --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $EVAL_ARGS |& tee "${WORKING_DIR}fixedwindow_eval_results.txt"
ENDBSUB
    ###### DFNRI – 2, 2
...
ENDBSUB
    ...
    DYNAMIC_MODEL_ARGS="--graph_type dynamic ${ENCODER_ARGS} ${DECODER_ARGS}"
    mkdir -p $WORKING_DIR
    bsub -J "motion_118" -n $NUM_CPU_CORES -W $RUN_TIME -R "rusage[mem=$MEM_PER_CPU_CORE,ngpus_excl_p=$NUM_GPU_CORES]" -R "select[gpu_mtotal0>=10240]" <<ENDBSUB
    python -u dnri/experiments/motion_experiment.py --gpu --mode train --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS |& tee "${WORKING_DIR}results.txt"
    python -u dnri/experiments/motion_experiment.py --gpu --mode eval_fixedwindow --load_best_model --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $EVAL_ARGS |& tee "${WORKING_DIR}fixedwindow_eval_results.txt"
    python -u dnri/experiments/motion_experiment.py --gpu --mode eval_fixedwindow --load_best_model --data_path $DATA_PATH --working_dir $WORKING_DIR $DYNAMIC_MODEL_ARGS $EVAL_ARGS --error_out_name prediction_errors_dynamic.npy |& tee "${WORKING_DIR}fixedwindow_eval_results_dynamic.txt"
ENDBSUB
    ###### NRI – 4
