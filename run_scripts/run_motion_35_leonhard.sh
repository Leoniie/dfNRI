...
do
    ...
    mkdir -p $WORKING_DIR
    bsub -J "motion_35" -n $NUM_CPU_CORES -W $RUN_TIME -R "rusage[mem=$MEM_PER_CPU_CORE,ngpus_excl_p=$NUM_GPU_CORES]" -R "select[gpu_mtotal0>=10240]" <<ENDBSUB
    python -u dnri/experiments/motion_experiment.py --gpu --mode train --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS |& tee "${WORKING_DIR}results.txt"
    python -u dnri/experiments/motion_experiment.py --gpu --mode eval --load_best_model --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS |& tee "${WORKING_DIR}eval_results.txt"
ENDBSUB
    ###### DFNRI – 2, 2
...
ENDBSUB
    ###### DNRI – 16, skip_first
    WORKING_DIR="${BASE_RESULTS_DIR}dnri_16_edge_skip_first/seed_${SEED}/"
    ENCODER_ARGS="--encoder_hidden 256 --encoder_mlp_num_layers 3 --encoder_mlp_hidden 128 --encoder_rnn_hidden 64"
    DECODER_ARGS="--decoder_hidden 256"
    HIDDEN_ARGS="--rnn_hidden 64"
    PRIOR_ARGS="--use_learned_prior --prior_num_layers 3 --prior_hidden_size 128"
    MODEL_ARGS="--model_type dnri --graph_type dynamic --skip_first --num_edge_types 16 $ENCODER_ARGS $DECODER_ARGS $HIDDEN_ARGS $PRIOR_ARGS --seed ${SEED}"
    TRAINING_ARGS='--val_batch_size 1 --batch_size 4 --accumulate_steps 2 --lr 5e-4 --use_adam --num_epochs 600 --lr_decay_factor 0.5 --lr_decay_steps 300 --normalize_kl --normalize_nll --tune_on_nll --val_teacher_forcing --teacher_forcing_steps -1'
    mkdir -p $WORKING_DIR
    bsub -J "motion_35" -n $NUM_CPU_CORES -W $RUN_TIME -R "rusage[mem=$MEM_PER_CPU_CORE,ngpus_excl_p=$NUM_GPU_CORES]" -R "select[gpu_mtotal0>=10240]" <<ENDBSUB
    python -u dnri/experiments/motion_experiment.py --gpu --mode train --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS |& tee "${WORKING_DIR}results.txt"
...
ENDBSUB
    ###### DNRI – 16
    WORKING_DIR="${BASE_RESULTS_DIR}dnri_16_edge/seed_${SEED}/"
    ENCODER_ARGS="--encoder_hidden 256 --encoder_mlp_num_layers 3 --encoder_mlp_hidden 128 --encoder_rnn_hidden 64"
    DECODER_ARGS="--decoder_hidden 256"
    HIDDEN_ARGS="--rnn_hidden 64"
    PRIOR_ARGS="--use_learned_prior --prior_num_layers 3 --prior_hidden_size 128"
    MODEL_ARGS="--model_type dnri --graph_type dynamic --num_edge_types 16 $ENCODER_ARGS $DECODER_ARGS $HIDDEN_ARGS $PRIOR_ARGS --seed ${SEED}"
    TRAINING_ARGS='--val_batch_size 1 --batch_size 4 --accumulate_steps 2 --lr 5e-4 --use_adam --num_epochs 600 --lr_decay_factor 0.5 --lr_decay_steps 300 --normalize_kl --normalize_nll --tune_on_nll --val_teacher_forcing --teacher_forcing_steps -1'
    mkdir -p $WORKING_DIR
    bsub -J "motion_35" -n $NUM_CPU_CORES -W $RUN_TIME -R "rusage[mem=$MEM_PER_CPU_CORE,ngpus_excl_p=$NUM_GPU_CORES]" -R "select[gpu_mtotal0>=10240]" <<ENDBSUB
    python -u dnri/experiments/motion_experiment.py --gpu --mode train --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS |& tee "${WORKING_DIR}results.txt"
...
ENDBSUB
    ...
    python -u dnri/experiments/motion_experiment.py --gpu --mode train --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS |& tee "${WORKING_DIR}results.txt"
    python -u dnri/experiments/motion_experiment.py --gpu --mode eval --load_best_model --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS |& tee "${WORKING_DIR}eval_results.txt"
    python -u dnri/experiments/motion_experiment.py --gpu --mode eval --load_best_model --data_path $DATA_PATH --working_dir $WORKING_DIR $DYNAMIC_MODEL_ARGS $TRAINING_ARGS --error_out_name prediction_errors_dynamic.npy |& tee "${WORKING_DIR}eval_results_dynamic.txt"
ENDBSUB
    ###### NRI – 4
...
ENDBSUB
    ###### NRI – 16, skip_first
    WORKING_DIR="${BASE_RESULTS_DIR}nri_16_edge_skip_first/seed_${SEED}/"
    ENCODER_ARGS='--num_edge_types 16 --encoder_hidden 256 --skip_first --encoder_mlp_hidden 256 --encoder_mlp_num_layers 3'
    DECODER_ARGS=''
    MODEL_ARGS="--model_type nri --graph_type static ${ENCODER_ARGS} ${DECODER_ARGS} --seed ${SEED}"
    TRAINING_ARGS='--batch_size 8 --lr 5e-4 --use_adam --num_epochs 600 --lr_decay_factor 0.5 --lr_decay_steps 300 --normalize_kl --normalize_nll --tune_on_nll --val_teacher_forcing'
    DYNAMIC_MODEL_ARGS="--model_type nri --graph_type dynamic ${ENCODER_ARGS} ${DECODER_ARGS}"
...
ENDBSUB
    ###### NRI – 16
    WORKING_DIR="${BASE_RESULTS_DIR}nri_16_edge/seed_${SEED}/"
    ENCODER_ARGS='--num_edge_types 16 --encoder_hidden 256 --encoder_mlp_hidden 256 --encoder_mlp_num_layers 3'
    DECODER_ARGS=''
    MODEL_ARGS="--model_type nri --graph_type static ${ENCODER_ARGS} ${DECODER_ARGS} --seed ${SEED}"
    TRAINING_ARGS='--batch_size 8 --lr 5e-4 --use_adam --num_epochs 600 --lr_decay_factor 0.5 --lr_decay_steps 300 --normalize_kl --normalize_nll --tune_on_nll --val_teacher_forcing'
    DYNAMIC_MODEL_ARGS="--model_type nri --graph_type dynamic ${ENCODER_ARGS} ${DECODER_ARGS}"
