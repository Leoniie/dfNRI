...
if __name__ == '__main__':
    ...
    args.layer_num_edge_types = [int(i) for i in args.layer_num_edge_types]
    params = vars(args)

    misc.seed(args.seed)

    # We require at least 2 layer-graphs when using dfNRI.
    if params['model_type'] == 'dfnri':
        if not len(params['layer_num_edge_types']) > 1:
            raise ValueError('At least 2 layer-graphs are required when using dfNRI.')
        # K = \sum_{1<=a<=n} K_a, see eq. (4) in fNRI paper
        params['num_edge_types'] = np.sum(params['layer_num_edge_types'])
    params['num_vars'] = params['num_agents'] = 11
