...
def train(model, train_data, val_data, params, train_writer, val_writer):
    ...
    train_data_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True, drop_last=True, pin_memory=gpu)
    val_data_loader = DataLoader(val_data, batch_size=val_batch_size, pin_memory=gpu)
    lr = params['lr']
    wd = params.get('wd', 0.)
