...
def eval_forward_prediction(model, dataset, burn_in_steps, forward_pred_steps, params, return_total_errors=False, path=None):
    ...
    for batch_ind, batch in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            all_gt_preds.append(gt_predictions.cpu().numpy())
            batch_count += 1
    ...
    # Save all predictions for later visualisation if a path is provided.
    if path is not None:
        np.save(os.path.join(path, 'model_predictions.npy'), np.array(all_model_preds))
        np.save(os.path.join(path, 'gt_predictions.npy'), np.array(all_gt_preds))
    if return_total_errors:
